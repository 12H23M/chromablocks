...
func rescue_existing_tray(tray: Array, board: BoardState, level: int) -> Dictionary:
	## Returns {"tray": Array, "rescued_indices": Array[int]}
	var rescued_indices: Array = []
	var new_tray: Array = tray.duplicate()

	# Find which pieces are unplaceable
	var unplaceable: Array = []
	for idx in new_tray.size():
		var piece: BlockPiece = new_tray[idx]
		var placeable := false
		for gy in board.rows:
			for gx in board.columns:
				if board.can_place_piece_at(piece, gx, gy):
					placeable = true
					break
			if placeable:
	...
	if unplaceable.is_empty():
		return {"tray": new_tray, "rescued_indices": []}

	# Try to rescue each unplaceable piece individually
	var downsize_map: Dictionary = {
		SizeCat.HUGE: SizeCat.LARGE,
		SizeCat.LARGE: SizeCat.MEDIUM,
		SizeCat.MEDIUM: SizeCat.SMALL,
	...
	for idx in unplaceable:
		...
		if cat in downsize_map:
			...
			for pt in pool:
				if _can_place_type_on_board(pt, board):
					var shape: Array = PieceDefinitions.SHAPES[pt]
					new_tray[idx] = BlockPiece.new(pt, _rng.randi_range(0, color_count - 1), shape)
		...
		# Level 2: FIT_SCAN
		for scan_cat in [SizeCat.TINY, SizeCat.SMALL, SizeCat.MEDIUM]:
			var pool: Array = CATEGORY_PIECES[scan_cat].duplicate()
			pool.shuffle()
			for pt in pool:
				if _can_place_type_on_board(pt, board):
					var shape: Array = PieceDefinitions.SHAPES[pt]
		...
		# Level 3: SINGLE_FORCE
		if _has_any_empty_cell(board):
			var shape: Array = PieceDefinitions.SHAPES[Enums.PieceType.SINGLE]
			new_tray[idx] = BlockPiece.new(Enums.PieceType.SINGLE, _rng.randi_range(0, color_count - 1), shape)
			rescued_indices.append(idx)

	return {"tray": new_tray, "rescued_indices": rescued_indices}
